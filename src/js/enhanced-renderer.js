...
class Renderer {
  constructor() {
    ...
    this.chordDetector = new ChordDetector(this.audioManager.getChordAnalyser(), {
      ...
      confEnter: CONFIG.CHORD_DETECTOR.CONF_ENTER,
      confExit: CONFIG.CHORD_DETECTOR.CONF_EXIT,
      holdMsEnter: CONFIG.CHORD_DETECTOR.HOLD_MS_ENTER,
      holdMsExit: CONFIG.CHORD_DETECTOR.HOLD_MS_EXIT,
      harmonicThreshold: CONFIG.CHORD_DETECTOR.HARMONIC_THRESHOLD,
      getRms: () => this.audioManager.getCurrentRms(),
      rmsGate: CONFIG.CHORD_DETECTOR.RMS_GATE
    });

    this.chordVisualizer = new ChordVisualizer('chord-visualization', {
      showCircleOfFifths: CONFIG.VISUALIZER.SHOW_CIRCLE_OF_FIFTHS,
      showPianoRoll: CONFIG.VISUALIZER.SHOW_PIANO_ROLL,
      showSpectralAnalysis: CONFIG.VISUALIZER.SHOW_SPECTRAL_ANALYSIS,
      animationDuration: CONFIG.VISUALIZER.ANIMATION_DURATION,
      colorScheme: CONFIG.VISUALIZER.COLOR_SCHEME
    });
    ...
    this.chordDetector.setOnChord((chordData) => {
      ...
      if (!chordData.name) {
        ...
        const { fg, bg } = this.getChordColors(chordData.name);
  ...
  update() {
    ...
    if (this.chordVisualizer) {
      this.chordVisualizer.updateChromaData(this.chordDetector.chromaEma);
    }
